...
**Project**: Shell Profile Manager (`profile` CLI)
**Language**: Go 1.21+
**Type**: CLI Tool for workspace environment management
**Repository**: github.com/mindmorass/shell-profile-manager
...
**Use SpecKit for**:
- New major commands (e.g., `profile sync`, `profile export`)
...
**For new command: `profile backup`**
...
```bash
# macOS
GOOS=darwin GOARCH=amd64 go build -o profile-darwin-amd64 ./cmd/profile

# Linux
GOOS=linux GOARCH=amd64 go build -o profile-linux-amd64 ./cmd/profile
```
...
# Running
./profile create work     # Create profile
./profile list            # List profiles
./profile select          # Interactive select
...
- **Binary**: `./profile`
- **Config**: `~/.config/profile/config.yaml`
- **Profiles**: `~/.config/profile/profiles/` (or user-configured)
