...
### Add Secrets (Not Tracked in Git)
...
These are automatically loaded when you enter the directory.
...
export PYTHONPATH="$WORKSPACE_HOME/lib"
```
...
```bash
# Automatically configured in .envrc
...
1Password SSH agent is automatically configured in new profiles, allowing you to use SSH keys stored in 1Password:
...
dotfiles/.config/1Password/agent.toml
```

To configure SSH keys from your 1Password vaults:
...
When the profile is active, the 1Password SSH agent will automatically load the configured keys for Git operations and SSH connections.
